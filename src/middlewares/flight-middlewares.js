...
/*    
    if(!req.body.boardingGate){
        ErrorResponse.message = 'Something went wrong creating flight';
        ErrorResponse.error = new AppError(['boardingGate not found in incoming request in correct form '],StatusCodes.BAD_REQUEST )
        return res
                .status(StatusCodes.BAD_REQUEST)
                .json(ErrorResponse);
    }
...
async function validateUpdateSeatsRequest(req, res, next){
    ...
    if(!req.body.seats){
        ErrorResponse.message = 'Something went wrong updating flight';
